...
function renderDomainDetails(data, block) {
    ...
    if (data.roadmap && data.roadmap.length > 0) {
        ...
        data.roadmap.forEach(step => {
            ...
            const videoUrl = getRoadmapVideoUrl(data.domainName || "", step.title || "", step.description || "");
            const videoHtml = videoUrl
                ? "<p><a href=\"" + encodeURI(videoUrl) + "\" target=\"_blank\" rel=\"noopener noreferrer\">Watch related video</a></p>"
                : "";
            div.innerHTML = "<strong>Step " + (step.stepOrder) + ": " + safeTitle + "</strong><p>" + safeDescription + "</p>" + videoHtml;
    ...
    if (data.certifications && data.certifications.length > 0) {
        ...
        data.certifications.forEach(c => {
            ...
            const certLink = getCertificationResourceLink(c.name || "", c.description || "");
            const linkHtml = certLink
                ? "<br><a href=\"" + encodeURI(certLink.url) + "\" target=\"_blank\" rel=\"noopener noreferrer\">" + escapeHtml(certLink.label) + "</a>"
                : "";
            div.innerHTML = "<strong>" + escapeHtml(c.name || "") + "</strong>" + (c.level ? " <span class=\"level\">(" + escapeHtml(c.level) + ")</span>" : "") + "<br>" + escapeHtml(c.description || "") + linkHtml;
...
function getRoadmapVideoUrl(domainName, stepTitle, stepDescription) {
    const query = [domainName, stepTitle, stepDescription]
        .filter(Boolean)
        .join(" ")
        .trim();
    if (!query) return "";
    return "https://www.youtube.com/results?search_query=" + encodeURIComponent(query + " tutorial for beginners");
}

function getCertificationResourceLink(certName, certDescription) {
    const combined = (certName + " " + certDescription).toLowerCase();

    if (combined.includes("aws")) {
        return { label: "Official AWS certification page", url: "https://aws.amazon.com/certification/" };
    }
    if (combined.includes("azure")) {
        return { label: "Official Microsoft certification page", url: "https://learn.microsoft.com/en-us/credentials/" };
    }
    if (combined.includes("google") || combined.includes("gcp")) {
        return { label: "Official Google Cloud certification page", url: "https://cloud.google.com/learn/certification" };
    }
    if (combined.includes("cisco") || combined.includes("ccna")) {
        return { label: "Official Cisco certification page", url: "https://www.cisco.com/site/us/en/learn/training-certifications/certifications.html" };
    }
    if (combined.includes("autocad")) {
        return { label: "Official Autodesk certification page", url: "https://www.autodesk.com/certification" };
    }
    if (combined.includes("solidworks")) {
        return { label: "Official SOLIDWORKS certification page", url: "https://www.solidworks.com/solidworks-certification-program" };
    }

    const searchText = (certName || certDescription || "").trim();
    if (!searchText) return null;
    return {
        label: "Find course/certification resources",
        url: "https://www.youtube.com/results?search_query=" + encodeURIComponent(searchText + " certification")
    };
}
